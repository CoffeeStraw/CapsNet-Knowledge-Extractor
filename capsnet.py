...
def CapsuleNet(input_shape, batch_size, n_class, r_iter=3, name="CapsNet_MNIST"):
    ...
    conv1 = Conv2D(input_shape=input_shape, data_format='channels_last',
                   filters=128, kernel_size=9, strides=1,
                   padding='valid', activation='relu', name='conv1')(x)

    # Layer 2: PrimaryCaps Layer
    primary_caps = PrimaryCaps(n_capsules=16, out_dim_capsule=8, kernel_size=9,
                               strides=2, padding='valid', name='primary_caps')(conv1)
    ...
    digit_caps = ClassCaps(
        n_capsules=n_class, out_dim_capsule=16, r_iter=r_iter, name='digit_caps')(primary_caps)
    ...
    decoder.add(Reshape(target_shape=input_shape,
